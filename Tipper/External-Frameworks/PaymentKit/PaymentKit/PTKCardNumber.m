...
- (NSString *)formattedString
{
    ...
    if (self.cardType == PTKCardTypeAmex) {
//        regex = [NSRegularExpression regularExpressionWithPattern:@"(\\d{1,4})(\\d{1,6})?(\\d{1,5})?" options:0 error:NULL];      // Marked by luokey
        regex = [NSRegularExpression regularExpressionWithPattern:@"(\\d{1,4})(\\d{1,4})?(\\d{1,4})?(\\d{1,3})?" options:0 error:NULL];
    } else {
