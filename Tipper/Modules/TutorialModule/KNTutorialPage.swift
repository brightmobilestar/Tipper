...
enum TutorialAnimationType{
    ...
    static func fromString(str:String)->TutorialAnimationType{
        switch(str){
        case "Linear":
            return .Linear
...
class KNTutorialPage: UIViewController  {
    ...
    func tutorialDidScroll(position: CGFloat, offset: CGFloat) {
        
        for(var i = 0; i < subsWeights.count ;i++){
            
            // Perform Transition/Scale/Rotate animations
            switch TutorialAnimationType.fromString(animationType){
                
            case TutorialAnimationType.Linear:
                animationLinear(i, offset)
                
            case TutorialAnimationType.Zoom:
                animationZoom(i, offset)
                
            case TutorialAnimationType.Curve:
                animationCurve(i, offset)
                
            case TutorialAnimationType.InOut:
