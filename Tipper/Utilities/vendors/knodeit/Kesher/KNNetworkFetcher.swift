...
extension KNKesherConfiguration {
    ...
    public struct KNNetworkFetcher {
...
public class KNNetworkFetcher<T : DataConvertible> : KNFetcher<T> {
    ...
    private func onReceiveData(data : NSData!, response : NSURLResponse!, error : NSError!, failure fail : ((NSError?) -> ()), success succeed : (T.Result) -> ()) {
        ...
        if let error = error {
            ...
            KNLogger.error("Request \(URL.absoluteString!) failed", error)
        ...
        if httpResponse == nil {
            KNLogger.error("Request \(URL.absoluteString!) received unknown response \(response)")
            return
        }
        ...
        if !httpResponse.validateLengthOfData(data) {
    ...
    private func failWithCode(code : KNKesherConfiguration.KNNetworkFetcher.ErrorCode, localizedDescription : String, failure fail : ((NSError?) -> ())) {
