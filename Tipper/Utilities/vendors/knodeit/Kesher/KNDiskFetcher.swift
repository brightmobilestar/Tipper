...
extension KNKesherConfiguration {
    ...
    //Swift doesn't allow to declare an enum in a generic type
    public struct KNDiskFetcher {
...
public class KNDiskFetcher<T : DataConvertible> : KNFetcher<T> {
    let path : String
    var cancelled = false
    ...
    private func privateFetch(failure fail : ((NSError?) -> ()), success succeed : (T.Result) -> ()) {
        ...
        if value == nil {
            ...
            let error = errorWithCode(KNKesherConfiguration.KNDiskFetcher.ErrorCode.InvalidData.rawValue, description: description)
            dispatch_async(dispatch_get_main_queue()) {
                fail(error)
            }
            return
